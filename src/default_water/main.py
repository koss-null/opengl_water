...
void main() {
    ...
    vec3 normal=normalize(v_normal);
    float diffused_intensity=u_diffused_mult * 0.3 * max(0, -dot(normal, u_sun_direction));
    float cosphi=max(0, dot(u_sun_direction, normalize(v_reflected)));
    float reflected_intensity=u_reflected_mult * pow(cosphi, 100) * 0.07;
    ...
    float refl = reflected_intensity;
    float diff = diffused_intensity;
    float u_sky_mlt = u_sky_mult;
    if (1-v_refl < 0) {
        v_refl = 0;
        refl = 0;
        diff = 0;
    ...
    vec3 rgb=u_sky_mlt * sky_color * v_reflectance + image_color * (1-v_refl) +
         diff*u_sun_diffused_color + refl*u_sun_reflected_color;
...
class Canvas(app.Canvas):
    def __init__(self, surface, sky="fluffy_clouds.png", bed="seabed.png"):
        ...
        self.program['u_bed_texture'] = gloo.Texture2D(self.bed, wrapping='clamp_to_edge', interpolation='linear')
        self.program_point["u_eye_height"] = self.program["u_eye_height"] = 3
        self.program["u_alpha"] = 0.9
        self.program["u_bed_depth"] = 1
        self.program["u_sun_direction"] = normalize([0, 0.5, 1])
    ...
    def apply_flags(self):
        ...
        self.program["u_reflected_mult"] = 0.7 if self.reflected_flag else 0
